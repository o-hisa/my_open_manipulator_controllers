...
HardwareInterface::HardwareInterface(ros::NodeHandle nh,
    ...
    : node_handle_(nh), priv_node_handle_(private_nh) {
  /************************************************************
  ** Initialize ROS parameters
  ************************************************************/
  port_name_ = priv_node_handle_.param<std::string>("usb_port", "/dev/ttyUSB0");
  baud_rate_ = priv_node_handle_.param<int32_t>("baud_rate", 4000000);
...
void HardwareInterface::registerActuatorInterfaces() {
  ...
  if (result == false) {
    ROS_ERROR(
        "Please check control table "
        "(http://emanual.robotis.com/#control-table)");
    return;
  }
...
bool HardwareInterface::initWorkbench(const std::string port_name,
...
bool HardwareInterface::getDynamixelsInfo(const std::string yaml_file) {
  ...
  for (YAML::const_iterator it_file = dynamixel.begin();
    ...
    for (YAML::const_iterator it_item = item.begin(); it_item != item.end();
         it_item++) {
...
bool HardwareInterface::loadDynamixels(void) {
  ...
  for (auto const& dxl : dynamixel_) {
    ...
    if (result == false) {
      ...
      ROS_INFO("Name : %s, ID : %d, Model Number : %d", dxl.first.c_str(),
               dxl.second, model_number);
    }
  }
...
bool HardwareInterface::initDynamixels(void) {
      ...
      if (dxl.first == info.first) {
        if (info.second.item_name != "ID" &&
          ...
          if (result == false) {
            ROS_ERROR("%s", log);
            ROS_ERROR(
                "Failed to write value[%d] on items[%s] to Dynamixel[Name : "
                "%s, ID : %d]",
  ...
  for (auto const& dxl : dynamixel_) dxl_wb_->torqueOn((uint8_t)dxl.second);
...
bool HardwareInterface::initControlItems(void) {
  ...
  const ControlItem* goal_position =
      dxl_wb_->getItemInfo(it->second, "Goal_Position");
  if (goal_position == NULL) return false;

  const ControlItem* goal_velocity =
      dxl_wb_->getItemInfo(it->second, "Goal_Velocity");
  ...
  const ControlItem* goal_current =
      dxl_wb_->getItemInfo(it->second, "Goal_Current");
  ...
  const ControlItem* present_position =
      dxl_wb_->getItemInfo(it->second, "Present_Position");
  if (present_position == NULL) return false;

  const ControlItem* present_velocity =
      dxl_wb_->getItemInfo(it->second, "Present_Velocity");
  ...
  const ControlItem* present_current =
      dxl_wb_->getItemInfo(it->second, "Present_Current");
...
bool HardwareInterface::initSDKHandlers(void) {
  ...
  result = dxl_wb_->addSyncWriteHandler(
      control_items_["Goal_Current"]->address,
      control_items_["Goal_Current"]->data_length, &log);
  if (result == false) {
    ROS_ERROR("%s", log);
    return result;
  } else {
  ...
  if (dxl_wb_->getProtocolVersion() == 2.0f) {
    ...
    /* As some models have an empty space between Present_Velocity and Present
     * Current, read_length is modified as below.*/
...
void HardwareInterface::registerControlInterfaces() {
  ...
  for (auto const& dxl : dynamixel_) {
    if (joint_size < (uint8_t)dxl.second) joint_size = (uint8_t)dxl.second;
  }
  ...
  for (auto iter = dynamixel_.begin(); iter != dynamixel_.end(); iter++) {
    ...
    ROS_INFO("joint_name : %s, servo ID: %d", iter->first.c_str(),
             iter->second);

    // connect and register the joint state interface
    hardware_interface::JointStateHandle joint_state_handle(
        iter->first.c_str(), &joints_[iter->second - 1].position,
        &joints_[iter->second - 1].velocity, &joints_[iter->second - 1].effort);
...
void HardwareInterface::read() {
  ...
  result =
      dxl_wb_->syncRead(sync_read_handler, id_array, dynamixel_.size(), &log);
  if (result == false) {
    ROS_ERROR("%s", log);
  }

  result = dxl_wb_->getSyncReadData(
      sync_read_handler, id_array, id_cnt,
      control_items_["Present_Current"]->address,
  ...
  for (uint8_t index = 0; index < id_cnt; index++) {
    // Position
    joints_[id_array[index] - 1].position = dxl_wb_->convertValue2Radian(
        (uint8_t)id_array[index], (int32_t)get_position[index]);
    if (strcmp(name_array[index].c_str(), "gripper") == 0)
      joints_[id_array[index] - 1].position /= 150.0;

    // Velocity
    joints_[id_array[index] - 1].velocity = dxl_wb_->convertValue2Velocity(
        (uint8_t)id_array[index], (int32_t)get_velocity[index]);

    // Effort
    if (strcmp(dxl_wb_->getModelName((uint8_t)id_array[index]), "XL-320") == 0)
      joints_[id_array[index] - 1].effort =
          dxl_wb_->convertValue2Load((int16_t)get_current[index]);
    else
      joints_[id_array[index] - 1].effort =
          dxl_wb_->convertValue2Current((int16_t)get_current[index]) *
          (1.78e-03);
...
void HardwareInterface::write() {
  ...
  if (strcmp(interface_.c_str(), "position") == 0) {
    for (auto const& dxl : dynamixel_) {
      id_array[id_cnt] = (uint8_t)dxl.second;
      dynamixel_position[id_cnt] = dxl_wb_->convertRadian2Value(
          (uint8_t)dxl.second,
          joints_[(uint8_t)dxl.second - 1].position_command);

      if (strcmp(dxl.first.c_str(), "gripper") == 0)
        dynamixel_position[id_cnt] = dxl_wb_->convertRadian2Value(
            (uint8_t)dxl.second,
            joints_[(uint8_t)dxl.second - 1].position_command * 150.0);
      id_cnt++;
    }
    uint8_t sync_write_handler = 0;  // 0: position, 1: velocity, 2: effort
    result = dxl_wb_->syncWrite(sync_write_handler, id_array, id_cnt,
                                dynamixel_position, 1, &log);
  } else if (strcmp(interface_.c_str(), "effort") == 0) {
    for (auto const& dxl : dynamixel_) {
      id_array[id_cnt] = (uint8_t)dxl.second;
      dynamixel_effort[id_cnt] = dxl_wb_->convertCurrent2Value(
          (uint8_t)dxl.second,
          joints_[(uint8_t)dxl.second - 1].effort_command / (1.78e-03));
